...
import logging
from utils import AverageMeter, time, print_log
...
def weights_FI_simulation(model: nn.Module, 
    ...
    with torch.no_grad():
        golden_predicted, golden_accuracy = model_evaluation(model, dataloader, device)
        ...
        for i in range(repetition_count):
            ...
            print_log("**********************************", logger)
            print_log(f"Iteration: [{i + 1:03d}/{repetition_count:03d}]", logger)

            # Inject faults
            counter = fault_counter()
            model_copy = weights_FI(model_copy, BER, device, counter)
            inject_time = time.time()
            attack_time.update(inject_time - start_time)
            faulty_params_count_total += counter.fault_count

            # Evaluate faulty model
            faulty_accuracy, DUE, SDC_critical, SDC_non_critical = faulty_model_evaluation(
                model_copy, dataloader, classes_count, golden_predicted, device
            )
            test_time = time.time()
            iter_time.update(test_time - start_time)
            ...
            # Print BFA-style stats
            total_bits = sum(p.numel() * 32 for p in model.parameters())
            bit_flip_pct = counter.fault_count / total_bits * 100
            print_log(f"bit flips: {counter.fault_count} ({bit_flip_pct:.6f}%)", logger)
            print_log(f"  **Test** Prec@1 {faulty_accuracy * 100:.3f} Error@1 {100 - faulty_accuracy * 100:.3f}", logger)
            print_log(f"iteration Time {iter_time.val:.3f} ({iter_time.avg:.3f})", logger)

            del model_copy
            model_copy = copy.deepcopy(model)
    
    print_log("\n\n" + f"average number of faults: {faulty_params_count_total / repetition_count}", logger)
    print_log(f"golden accuracy: {golden_accuracy * 100}%", logger)
    print_log(f"weight FI, average accuracy: {faulty_accuracy_total * 100 / (repetition_count)}%", logger)
    print_log(f"average DUE: {DUE_total * 100 / repetition_count}%", logger)
    print_log(f"average critical SDC: {(SDC_critical_total) * 100 / repetition_count}%", logger)
    print_log(f"average non-critical SDC: {SDC_non_critical_total * 100 / repetition_count}", logger)